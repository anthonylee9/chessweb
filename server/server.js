...
io.on('connection', (socket) => {
  ...
  socket.on("chatMessage2", (chatMessage2) => {
    io.emit("chatMessage2", chatMessage2); // Emitting message to all clients
  });
  ...
  socket.on('fetchLobbies', () => {
    const onePlayerLobbies = Array.from(rooms.values())
      .filter(room => room.players.length === 1);

    socket.emit('lobbiesUpdate', onePlayerLobbies);
  });
  ...
  socket.on('chatMessage', ({ room, message }) => {
    // Log room and message to verify they are correct
    console.log(`Received message in room ${room}: ${message}`);
  
    // Broadcast the chat message to all players in the room
    io.to(room).emit('chatMessage', { sender: socket.data.username, message });
  });
