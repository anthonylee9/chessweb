...
def get_move():
    try:
        ...
        fen = data['fen']
        board = chess.Board(fen)  # Convert FEN to board
        max_depth = 3  # Set the maximum depth of your search
        time_limit = 10  # Set a time limit for the search, in seconds

        best_move = find_best_move(board, max_depth, time_limit)  # Calculate the best move

        if best_move is not None:
            best_move_san = board.san(best_move)  # Convert the move to Standard Algebraic Notation (SAN) for easier understanding
            return jsonify({"best_move": best_move_san})
        else:
            return jsonify({"error": "Could not calculate the best move"}), 500
